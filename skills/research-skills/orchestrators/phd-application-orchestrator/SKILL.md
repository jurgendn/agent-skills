...
If motivation or direction is wobbly (the "should I even do a PhD" layer), route to `phd-motivation-keeper` before the package work — a package built on an unclear why reads hollow.
...
### Stage 1 — Read the profile → `applicant-profile-reader`
...
### Stage 2 — Build the CV → `academic-cv-builder`
...
### Stage 3 — Map program & faculty fit → `phd-program-fit-mapper`
...
### Stage 4 — Write the SOP → `sop-writer`
...
### Stage 5 — Plan recommenders → `recommendation-letter-strategist`
...
### Stage 6 — Professor outreach → `professor-cold-email-drafter`
...
### Stage 7 — Audit the package → `phd-application-package-auditor`
...
- **Motivation underneath.** If the "why" is unstable, route to `phd-motivation-keeper` before or alongside the package work.
