---
name: argument-audit
description: Domain-general check of whether the stated evidence/premises actually establish a claim — for a math proof, an essay paragraph, a research conclusion, or any everyday argument. Use when the user asks "does my evidence support this claim", "is this reasoning valid", "check the logic of this argument", "is this a non sequitur", "does the conclusion follow", "poke holes in my reasoning", "is this argument sound", or hands over a claim plus the statements meant to back it. Runs INTERACTIVELY and SOCRATICALLY: it walks the user to the weak link with one question at a time rather than dumping a verdict, then converges on a validity verdict + smallest fix or counterexample. Distinct from theorem-and-claim-audit (that is a direct, report-style audit specialized to formal math/ML paper claims — use it when the user wants the verdict handed to them, or the content is heavily mathematical/empirical); from citation-auditor (does a cited SOURCE actually say this — external attribution, not internal logic); from paper-argument-planner (build a paper's whole claim spine, not check one argument); and from professor-critic (grade a finished artifact against a named reader). Reach for this skill for the general "is this reasoning valid" question across any domain.
---
