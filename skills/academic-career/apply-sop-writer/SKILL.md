...
- **Build from verified differentiators and outcomes.** Use a programme profile from `apply-program-theme-extractor` (pass C) where one exists; otherwise search the web for the programme's learning outcomes, graduate profile, and what it offers, recording URL and access date. Use only differentiators backed by a page — a named partner, a mobility structure, a compulsory component, a facility — and never an unbacked claim.
