...
- `skills/research-skills/experiment-design-and-analysis/` contains experiment design, benchmark/baseline selection, statistical testing, evaluation, ML, and LLM-systems evaluation skills.
...
- `skills/research-skills/orchestrators/` contains lightweight router/meta-skills that sequence the singleton research skills into a guided workflow with stage gates, delegating the actual work to the singletons rather than duplicating it: `learn-new-topic` (breadth-first onboarding into an unfamiliar area), `deep-understanding` (depth-first mastery of one paper/method/theorem), `idea-to-proof` (raw idea → formal claim → proof design), `paper-lifecycle-orchestrator` (idea → experiments → writing → submission → rebuttal → artifact release), and `phd-application-orchestrator` (profile → CV → fit → SOP → recommenders → outreach → package audit). Orchestrators decide *what next* and which sub-skill to call; they never re-teach what a singleton already does.
